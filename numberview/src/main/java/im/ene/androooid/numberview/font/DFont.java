package im.ene.androooid.numberview.font;
...
import im.ene.androooid.numberview.DrawingHelper;
import im.ene.androooid.numberview.Font;
...
public class DFont implements Font {
    ...
    public void save(float[] a, float[] b, float percent, float[] result) {
        if (a == null || b == null || result == null || a.length != b.length || a.length > result.length)
            return;
